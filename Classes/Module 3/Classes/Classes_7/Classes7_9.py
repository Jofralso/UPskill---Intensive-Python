...
#Passa a (self,N) e igualamos a N
    ...
    def ClassFinal(self):
        CF = math.floor((self.__Teste1 + self.__Teste2) / 2 + 0.5)
        if CF>14:
            EstudanteInf.TSup14+=1
        Situacao=EstudanteInf.SitFinal(CF)
        return CF, Situacao

    def Impressao(self):
        print(f"{'Nome':^15}{'Teste 1':^10}{'Teste 2':^10}{'Classif. Final':^15}")
        print(f"{self.Nome:<15}{self.Teste1:^10}{self.Teste2:^8}{self.ClassFinal():^16.0f}")
        return